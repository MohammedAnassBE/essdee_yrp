...
def get_parent_ref(doc):
	...
	if doc.cutting_plan:
		return ('SD YRP Cutting Plan', doc.cutting_plan)
	elif doc.cutting_order:
		return ('SD YRP Cutting Order', doc.cutting_order)
...
def get_detail_doc(parent_dt, parent_name):
	...
	if parent_dt == 'SD YRP Cutting Plan':
		pd = frappe.get_value('SD YRP Cutting Plan', parent_name, "production_detail")
		return frappe.get_cached_doc('YRP Item Production Detail', pd)
	else:
		cod_name = frappe.get_value('SD YRP Cutting Order', parent_name, "cutting_order_detail")
		return frappe.get_cached_doc('SD YRP Cutting Order Detail', cod_name)


def validate_parent_status(parent_dt, parent_name):
	"""Validates parent is submitted and ready for cutting."""
	if parent_dt == 'SD YRP Cutting Plan':
		status, docstatus = frappe.get_value('SD YRP Cutting Plan', parent_name, ["cp_status", "docstatus"])
		if docstatus == 0:
			frappe.throw("Cutting Plan was not Submitted")
		if status == 'Planned':
			frappe.throw("Cloths Not Received in Cutting Plan")
	else:
		docstatus = frappe.get_value('SD YRP Cutting Order', parent_name, "docstatus")
		if docstatus != 1:
...
def get_parent_context(parent_dt, parent_name):
	...
	if parent_dt == 'SD YRP Cutting Plan':
		doc = frappe.get_doc('SD YRP Cutting Plan', parent_name)
		return {
			"parent_dt": parent_dt,
		...
		doc = frappe.get_doc('SD YRP Cutting Order', parent_name)
		cod = frappe.get_cached_doc('SD YRP Cutting Order Detail', doc.cutting_order_detail)
		return {
			"parent_dt": parent_dt,
			"parent_name": parent_name,
			"item": doc.item,
...
def increment_lay_no(parent_dt, parent_name, new_lay_no):
	...
	if parent_dt == 'SD YRP Cutting Plan':
		...
		frappe.db.set_value('SD YRP Cutting Order', parent_name, "lay_no", new_lay_no, update_modified=False)
...
def update_parent_status_on_first_lay(parent_dt, parent_name):
	...
	if parent_dt == 'SD YRP Cutting Plan':
		frappe.db.sql(
			f"""
				UPDATE `tabSD YRP Cutting Plan` SET cp_status = 'Cutting In Progress'
				WHERE name = {frappe.db.escape(parent_name)}
			"""
		)
	else:
		frappe.db.set_value('SD YRP Cutting Order', parent_name, "co_status", "Cutting In Progress", update_modified=False)


def has_cloth_tracking(parent_dt):
	"""Returns True for CP (has cutting_plan_cloth_details), False for CO."""
	return parent_dt == 'SD YRP Cutting Plan'
...
def is_parent_completed(parent_dt, parent_name):
	...
	if parent_dt == 'SD YRP Cutting Plan':
		status = frappe.get_value('SD YRP Cutting Plan', parent_name, "cp_status")
		return status == "Completed"
	else:
		status = frappe.get_value('SD YRP Cutting Order', parent_name, "co_status")
		return status == "Completed"
...
def save_completed_incomplete_json(parent_dt, parent_name, completed_items, incomplete_items):
	...
	if parent_dt == 'SD YRP Cutting Plan':
		...
		frappe.db.set_value('SD YRP Cutting Order', parent_name, {
...
def get_marker_parent_field(parent_dt):
	...
	return "cutting_plan" if parent_dt == 'SD YRP Cutting Plan' else "cutting_order"
