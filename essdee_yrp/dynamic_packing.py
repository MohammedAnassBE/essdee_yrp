"""Transaction-level packing ratios used by Essdee finishing flows.

This module owns only the company packing contract. Stock posting remains in
YRP's dimension-aware transaction controllers.
"""
...
def normalize_packing_batches(
	...
	"""Validate GRN-time packing rows and calculate exact piece quantities."""
	...
	for index, raw in enumerate(batches, 1):
		...
		for size in valid_sizes:
			...
			if qty < 0 or qty != cint(qty):
				frappe.throw(
					f"Packing ratio {index}, {size}: quantity should be a whole number"
				)
			if qty:
				ratio[size] = cint(qty)
		...
		unknown_sizes = [
			size
			for size, qty in raw_ratio.items()
			if flt(qty) and size not in valid_size_set
		]
		if unknown_sizes:
			frappe.throw(
...
def aggregate_batch_pieces(batches, box_field="box_quantity"):
	"""Return size pieces, physical boxes, and pieces for packing batches."""
...
def is_batch_tracked_packing_grn(grn):
	"""Return whether a GRN has auditable current or migrated packing batches."""
	version = grn.get("packing_calculation_version") if hasattr(grn, "get") else 0
