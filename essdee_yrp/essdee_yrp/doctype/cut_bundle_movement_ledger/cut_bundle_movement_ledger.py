...
def _transaction_bundle_quantities(doc, ipd_doc):
	...
	"""
	variant_field = "item" if doc.doctype == "Stock Entry" else "item_variant"
	quantity_field = {
		"Delivery Challan": "delivered_quantity",
		"Goods Received Note": "quantity",
		"Stock Entry": "qty",
	}.get(doc.doctype)
...
def get_cut_bundle_entry(cpm_doc, doc, target_warehouse, multiplier, cancelled=0):
	...
	ipd, item = frappe.get_value(
		"Lot", cpm_doc.lot, ["production_detail", "item"]
	) or (None, None)
...
def get_future_entry(entry, collapsed_bundle=0):
	...
	future = frappe.db.sql(
		"""
			SELECT name, quantity_after_transaction, set_combination FROM `tabCut Bundle Movement Ledger`
...
def _collapsed_transaction_rows(doc, lot, source_location, attrs):
	...
	for row in doc.get("items") or []:
		...
		if quantity <= 0:
			continue
		variant = row.get(variant_field)
		item = frappe.db.get_value("Item Variant", variant, "item")
		...
		if doc.doctype == "Stock Entry" and not combination:
...
def update_collapsed_bundle(doctype, docname, event, non_stich_process=False):
	...
	if not ipd or not frappe.db.exists("Cut Bundle Movement Ledger", {"lot": lot}):
		return
	fields = [
	...
	primary, packing, stitching, stitching_stage, dependent = frappe.db.get_value(
		"Item Production Detail", ipd, fields
	)
	attrs = {
		"primary": primary,
		"pack": packing,
