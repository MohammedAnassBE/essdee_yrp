...
from essdee_yrp.essdee_yrp.doctype.cutting_marker.cutting_marker import fetch_marker_details

from essdee_yrp.essdee_yrp.doctype.cut_bundle_movement_ledger.cut_bundle_movement_ledger import cancel_cut_bundle_ledger, make_cut_bundle_ledger
...
def _lock_cutting_laysheet(doc_name):
	frappe.db.sql(
		"SELECT name FROM `tabCutting LaySheet` WHERE name = %s FOR UPDATE",
		(doc_name,),
	)
...
def _bundle_movement_enabled(cls_doc):
	cancelled = frappe.db.get_single_value("MRP Settings", "cut_bundle_cancelled_lot")
	cancelled_lots = {
...
class CuttingLaySheet(Document):
	...
	def _validate_bulk_editor_lock(self):
		...
		if lot_transfer and frappe.db.get_value(
			"Lot Transfer", lot_transfer, "docstatus"
		) != 2:
			frappe.throw(
...
def get_select_attributes(cutting_plan=None, cutting_order=None):
	parent_dt = "Cutting Order" if cutting_order else "Cutting Plan"
...
def _save_cutting_plan_cloth_usage(laysheet_doc, cutting_plan):
	...
	for name in tracked_laysheets:
		doc = laysheet_doc if name == laysheet_doc.name else frappe.get_doc("Cutting LaySheet", name)
...
def print_labels(print_items, lay_no, doc_name, print_order, cutting_plan=None, cutting_order=None):
	...
	if cls.get("cutting_bulk_lay_sheet"):
		from essdee_yrp.essdee_yrp.doctype.cutting_bulk_lay_sheets.cutting_bulk_lay_sheets import (
...
def get_input_fields(cutting_marker, colour, select_attributes):
	frappe.get_doc("Cutting Marker", cutting_marker).check_permission("read")
	inputs = []
...
def approve_grammage(doc_name):
	...
	frappe.get_doc("Cutting LaySheet", doc_name).check_permission("write")
	settings = frappe.get_single("MRP Settings")
	approval_roles = [
		row.role for row in (settings.get("cls_grammage_approval_roles") or [])
	]
...
def revert_labels(doc_name):
	...
	is_moved_list = frappe.db.sql(
		"""
			SELECT name FROM `tabCutting LaySheet Bundle` WHERE parent = %s AND is_moved = 1
		""",
		(doc_name,),
		as_list=True,
	)
	...
	acc_moved_list = frappe.db.sql(
		"""
			SELECT name FROM `tabCutting LaySheet Accessory Detail` WHERE parent = %s AND moved_weight > 0
		""",
		(doc_name,),
		as_list=True,
	)
	...
	if cls_doc.cutting_plan:
		from essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan import calculate_laysheets
...
def create_grn_entry(doc_name):
	...
	item_name = frappe.get_value("Lot", cls_doc.lot, "item")
