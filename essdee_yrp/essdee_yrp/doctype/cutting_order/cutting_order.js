...
frappe.ui.form.on("Cutting Order", {
	refresh(frm) {
		...
		if (frm.doc.docstatus === 1) {
			...
			frappe.call({
				method: "essdee_yrp.essdee_yrp.doctype.cutting_order.cutting_order.get_cutting_order_planned_vs_actual",
			...
			frm.add_custom_button("Calculate LaySheets", function () {
				...
				frappe.call({
					method: "essdee_yrp.essdee_yrp.doctype.cutting_order.cutting_order.calculate_laysheets",
					args: {
	...
	cutting_order_detail(frm) {
		...
		frappe.call({
			method: 'essdee_yrp.essdee_yrp.doctype.cutting_order.cutting_order.get_cutting_order_detail_data',
