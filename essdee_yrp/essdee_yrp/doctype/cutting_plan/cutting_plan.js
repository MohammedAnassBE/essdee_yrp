...
frappe.ui.form.on("Cutting Plan", {
    ...
    refresh(frm) {
        ...
        if (frm.is_new()) {
            ...
            frm.add_custom_button("Generate", function () {
                ...
                frappe.call({
                    method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.get_cloth1",
                    args: {
            ...
            frm.add_custom_button("Fetch Received Cloth", () => {
                frappe.call({
                    method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.fetch_received_cloth",
                    args: {
            ...
            frm.add_custom_button("Calculate LaySheets", function () {
                ...
                frappe.call({
                    method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.calculate_laysheets",
                    args: {
                        cutting_plan: frm.doc.name,
                    },
                })
            ...
            frm.add_custom_button("Update Recut", () => {
                let d = new frappe.ui.Dialog({
                    ...
                    primary_action() {
                        ...
                        frappe.call({
                            method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.create_recut_print_panel",
                            args: {
                                cutting_plan: frm.doc.name,
                                type: "Recut",
                                item_details: items
    ...
    lot(frm) {
        if (frm.doc.lot) {
            frappe.call({
                method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.get_items",
                args: {
                    lot: frm.doc.lot,
                },
                callback: function (r) {
    ...
    generate_report(frm) {
        frappe.call({
            method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.get_cutting_plan_laysheets_report",
            args: {
                cutting_plan: frm.doc.name,
                production_detail: frm.doc.production_detail,
            }
...
function add_lot_transfer_button(frm) {
	if (frm.is_new() || frm.doc.docstatus !== 1 || !frappe.model.can_create("Lot Transfer")) {
		return;
	}
	...
	frm.add_custom_button(__("Lot Transfer"), () => {
		const dialog = new frappe.ui.Dialog({
			...
			primary_action(values) {
				frappe.call({
					method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.create_balance_lot_transfer",
					args: {
						cutting_plan: frm.doc.name,
...
function remove_cancelled_cutting_plan_actions(frm) {
	...
	frm.remove_custom_button("Update Recut", "Recut and Print Panel")
	frm.remove_custom_button("Update Print Panel", "Recut and Print Panel")
	frm.remove_custom_button("Fetch and Calculate")
	frm.remove_custom_button("Get or Update Completed")
	frm.remove_custom_button("Recut and Print Panel")
}
...
function add_change_approval_grammage_button(frm) {
    ...
    frappe.call({
        method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.can_change_approval_grammage",
        callback: function(r) {
            if (!r.message) {
                return;
            }
            ...
            frm.add_custom_button(__("Change Approval Grammage"), () => {
                let d = new frappe.ui.Dialog({
                    ...
                    primary_action(values) {
                        frappe.call({
                            method: "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.change_approval_grammage",
                            args: {
