...
def _get_submitted_cutting_plan(name, permission="write"):
	doc = frappe.get_doc("Cutting Plan", name)
...
def calculate_laysheets(cutting_plan, ignore_phantom_panels=False):
	...
	if phantom_panels:
		manual_trigger = frappe.form_dict.get("cmd") == "essdee_yrp.essdee_yrp.doctype.cutting_plan.cutting_plan.calculate_laysheets"
...
def get_cutting_plan_laysheets_report(cutting_plan):
	frappe.get_doc("Cutting Plan", cutting_plan).check_permission("read")
	cp_doc = frappe.get_doc("Cutting Plan", cutting_plan)
...
def get_ccr(doc_name):
	frappe.get_doc("Cutting Plan", doc_name).check_permission("read")
...
def create_balance_lot_transfer(cutting_plan, to_lot):
	...
	frappe.get_doc("Lot", to_lot).check_permission("read")
	if not cp_doc.work_order:
		frappe.throw(_("Work Order is required in the Cutting Plan."))

	supplier = frappe.get_cached_value("Work Order", cp_doc.work_order, "supplier")
	if not supplier:
