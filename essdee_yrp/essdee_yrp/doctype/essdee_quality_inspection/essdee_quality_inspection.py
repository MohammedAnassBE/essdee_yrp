...
def get_debit_details(quality_inspection: str) -> list[dict]:
	...
	if not frappe.get_meta("Debit").has_field("quality_inspection"):
		return []
	if not frappe.has_permission("Debit", "read"):
		return []
...
def get_default_aql_level() -> dict:
	...
	return {
		"major": frappe.db.get_single_value(
			"MRP Settings", "default_major_aql_level"
		),
		"minor": frappe.db.get_single_value(
			"MRP Settings", "default_minor_aql_level"
		),
	}
...
def create_inspection_debit(
	...
	inspection = frappe.get_doc("Essdee Quality Inspection", quality_inspection)
...
def _require_quality_access() -> None:
	if not (
		frappe.has_permission("Essdee Quality Inspection", "read")
		or frappe.has_permission("Essdee Quality Inspection", "create")
	):
...
def _get_work_order(against: str, against_id: str):
	if against != "Work Order" or not against_id:
		frappe.throw(_("Quality Inspection must be against a Work Order."))
	work_order = frappe.get_doc("Work Order", against_id)
...
def _matching_aql_row(aql_level: str, offer_qty: int):
	doc = frappe.get_cached_doc("AQL Level", aql_level)
