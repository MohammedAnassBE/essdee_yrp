...
from essdee_yrp.essdee_yrp.doctype.essdee_quality_inspection.essdee_quality_inspection import (
	_get_aql_limits,
...
class TestEssdeeQualityInspection(IntegrationTestCase):
	def test_quality_schema_defaults_and_debit_mapping(self):
		settings = frappe.get_meta("MRP Settings")
		...
		self.assertEqual(
			frappe.db.get_single_value("MRP Settings", "default_major_aql_level"),
			"Level-2.5",
		)
		self.assertEqual(
			frappe.db.get_single_value("MRP Settings", "default_minor_aql_level"),
			"Level-4.0",
		)

		debit_field = frappe.get_meta("Debit").get_field("quality_inspection")
		self.assertIsNotNone(debit_field)
		self.assertEqual(debit_field.options, "Essdee Quality Inspection")
	...
	def test_selected_order_quantity_matches_current_migrated_oracles(self):
		...
		for name in names:
			inspection = frappe.get_doc("Essdee Quality Inspection", name)
			work_order = frappe.get_doc("Work Order", inspection.against_id)
			self.assertEqual(
	...
	def test_quality_endpoints_require_login(self):
		...
		module = (
			"essdee_yrp.essdee_yrp.doctype.essdee_quality_inspection."
	...
	def test_inspection_debit_uses_mapped_base_debit(self):
		...
		inspection = frappe.get_doc("Essdee Quality Inspection", inspection_name)
		...
		debit = frappe.get_doc("Debit", result["name"])
