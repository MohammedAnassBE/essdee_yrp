...
from yrp.yrp.doctype.item.item import get_or_create_variant, rename_item
...
def sync_fg_items(names, rename=False):
	...
	for name in names:
		frappe.get_doc("FG Item Master", name).check_permission("write")
	job = frappe.enqueue(
		"essdee_yrp.essdee_yrp.doctype.fg_item_master.fg_item_master.sync_fg_items_background",
		queue="long",
		names=names,
		rename=cint(rename),
...
def sync_fg_items_background(names, rename=False):
	...
	for name in names or []:
		try:
			frappe.get_doc("FG Item Master", name).sync_item(rename=cint(rename))
		except Exception:
