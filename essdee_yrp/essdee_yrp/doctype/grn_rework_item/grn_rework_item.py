...
def sync_grn_rework(doc, method=None):
	...
	if isinstance(doc, str):
		doc = frappe.get_doc("Goods Received Note", doc)
	...
	for row in doc.get("items") or []:
		...
		if quantity <= QTY_TOLERANCE or received_type in {accepted, rejected}:
			continue
		if frappe.db.exists("GRN Rework Item Detail", {"source_grn_item": row.name}):
			continue
		groups.setdefault(row.get("row_index") or row.name, []).append(row)

	created = []
	for rows in groups.values():
		rework = frappe.new_doc("GRN Rework Item")
		rework.update(
			{
				"naming_series": "RW-",
				"grn_number": doc.name,
...
def sync_grn_rework_for_name(grn_number):
	...
	grn = frappe.get_doc("Goods Received Note", grn_number)
	grn.check_permission("read")
	if not frappe.has_permission("GRN Rework Item", "create"):
...
def before_grn_cancel(doc, method=None):
	...
	for name in frappe.get_all(
		"GRN Rework Item", filters={"grn_number": doc.name}, pluck="name"
	):
		rework = frappe.get_doc("GRN Rework Item", name)
...
def on_grn_cancel(doc, method=None):
	...
	for name in frappe.get_all(
		...
		frappe.delete_doc("GRN Rework Item", name, ignore_permissions=True)


def _page_permissions():
	return {
		"can_read": bool(frappe.has_permission("GRN Rework Item", "read")),
		"can_write": bool(frappe.has_permission("GRN Rework Item", "write")),
	}
...
def get_rework_items(
	...
	for name in frappe.get_all(
		"GRN Rework Item", filters=filters, pluck="name", order_by="creation desc"
	):
...
def _load_action_rows(payload):
	...
	parent = frappe.get_doc("GRN Rework Item", parents.pop(), for_update=True)
...
def _source_dimensions(parent, row, dimension_fieldnames):
	...
	if row.get("source_grn_item") and frappe.db.exists(
		"Goods Received Note Item", row.source_grn_item
	):
		source = frappe.get_doc("Goods Received Note Item", row.source_grn_item)
...
def _source_warehouse(parent, row, dimensions, qty, get_stock_balance):
	...
	if parent.grn_number:
		final_warehouse = frappe.db.get_value(
			"Goods Received Note", parent.grn_number, "to_warehouse"
		)
...
def _rebuild_finishing(lot):
	...
	for name in frappe.get_all("Finishing Plan", filters={"lot": lot}, pluck="name"):
		rebuild_finishing_plan(name, check_permission=False)


@frappe.whitelist()
def download_xl(data):
	if not frappe.has_permission("GRN Rework Item", "read"):
	...
	sheet.append(
		["Series No", "Date", "GRN Number", "Lot", "Item", "Colour", *types, "Total"]
	)
