...
def get_ipd_compacting(item_production_detail):
	name = frappe.db.exists("IPD Compacting", {"item_production_detail": item_production_detail})
...
class IPDCompacting(Document):
	def validate(self):
		ipd_doc = frappe.get_cached_doc("Item Production Detail", self.item_production_detail)
