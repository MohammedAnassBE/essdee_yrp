...
function build_cloth_programs_dialog(frm, cloths, defaults = {}) {
	const fields = [{
		...
		default: 0,
		description: __(
			"Added to every calculated knitting-program route. The finished-cloth requirement stays unchanged, and the Knitting Process default excess is not used."
		),
	}];
	cloths.forEach((c, i) => {
		...
		if (required_colours.length) {
			fields.push({
				...
				options: `<div class="text-muted small" style="margin-bottom:8px">${__(
					"The Item Yarn Recipe is applied automatically. Set the physical Knitting output colour and Dia for every finished route."
				)}</div>`,
			});
