...
PRODUCT_ATTACHMENT_FIELDS = {
	"Product": {"product_image", "top_image", "bottom_image", "front_image", "back_image"},
...
def _validate_submitted_measurement(name: str | None, label: str):
	if name and frappe.db.get_value("Product Measurement", name, "docstatus") != 1:
...
def _measurement_rows(name: str | None, group: str) -> list[dict]:
	if not name:
		return []
	doc = frappe.get_doc("Product Measurement", name)
	return [
...
def upload_product_file():
	product = _get_writable_doc("Product", frappe.form_dict.docname)
	content, original_filename = _uploaded_file_parts()
	upload_type = _requested_upload_type(product.name, original_filename)
	if not frappe.db.exists("Product Upload Type", upload_type):
...
def remove_graphic_image(detail, docname: str):
	product = _get_writable_doc("Product", docname)
	...
	if file_name and frappe.db.exists("File", file_name):
		...
		_assert_file_attached_to(file_doc, "Product", product.name)
...
def _copy_release_attachment(file_name: str | None, release_name: str):
	if not file_name:
		return None
	file_doc = frappe.get_doc("File", file_name)
	_assert_file_attached_to(file_doc, "Product", frappe.db.get_value("Product Release", release_name, "product"))
	return file_doc.create_attachment_copy(
		"Product Release", release_name, "file", ignore_permissions=True
	)


@frappe.whitelist()
def release_tech_pack(doc_name: str):
	product = _get_writable_doc("Product", doc_name)
	frappe.has_permission("Product Release", "create", throw=True)
	...
	release = frappe.new_doc("Product Release")
	for fieldname in (
		"is_cord",
		"item_name",
		"style_no",
...
def process_pdf_to_images(file_url: str, docname: str, table_name: str):
	...
	file_doc, pdf = _validated_pdf(file_url, "Product", docname, table_name)
