...
class SDYRPBoxStickerPrint(Document):
...
def get_fg_details(fg_item, lot=None):
	frappe.get_doc('SD YRP FG Item Master', fg_item).check_permission("read")
	sizes, mrp = frappe.db.get_value(
		'SD YRP FG Item Master', fg_item, ["available_sizes", "mrp"]
	) or ("", "")
...
def get_print_format(doc, print_items, printer_type):
	doc = frappe.get_doc('SD YRP Box Sticker Print', doc)
	doc.check_permission("write")
	if doc.docstatus != 1:
		frappe.throw(_("Submit the Box Sticker Print before printing"))
	_lock_linked_production_order(doc.lot)
	print_format = frappe.get_doc('YRP ZPL Raw Print Format', doc.print_format)
	...
	for item in items or []:
		...
		row = frappe.db.sql(
			"""
				...
				FROM `tabSD YRP Box Sticker Print Detail`
				WHERE name = %s
				FOR UPDATE
			""",
			(item.get("doc_name"),),
...
def get_template(doc, item, raw_code, label_count, fg_item):
	...
	if not doc.use_item_name:
		fg_item = frappe.db.get_value('SD YRP FG Item Master', fg_item, "display_name") or fg_item
...
def override_print_quantity(print_items, print_format):
	items = frappe.parse_json(print_items) if isinstance(print_items, str) else print_items
	format_doc = frappe.get_doc('YRP ZPL Raw Print Format', print_format)
	...
	for item in sorted(items or [], key=lambda value: value.get("doc_name") or ""):
		row = frappe.db.sql(
			"""
				...
				FROM `tabSD YRP Box Sticker Print Detail` d
				WHERE d.name = %s
				FOR UPDATE
			""",
			(item.get("doc_name"),),
		...
		if not row:
			frappe.throw(_("Invalid Box Sticker Print detail"))
		parent = frappe.get_doc('SD YRP Box Sticker Print', row[0].parent)
...
def get_raw_code(doc_name):
	doc = frappe.get_doc('SD YRP Box Sticker Print', doc_name)
	doc.check_permission("read")
	print_format = frappe.get_doc('YRP ZPL Raw Print Format', doc.print_format)
	print_format.check_permission("read")
...
def get_printer(printers):
	...
	configured = frappe.db.get_single_value('SD YRP MRP Settings', "printer_list")
	if not configured:
		return available
...
BoxStickerPrint = SDYRPBoxStickerPrint
