...
frappe.ui.form.on("SD YRP Cutting Bulk Lay Sheets", {
    setup(frm) {
...
frappe.ui.form.on("SD YRP Cutting Bulk Lay Sheet Detail", {
    lot(frm, cdt, cdn) {
        ...
        return `
            <div role="button" tabindex="0" class="bulk-lot-card ${active ? "is-active" : ""}"
                ...
                <div class="bulk-card-grid">
                    ...
                    <span>Lay Sheet</span><span>${bulk_route_link("SD YRP Cutting LaySheet", row.cutting_laysheet)}</span>
                    <span>Transfer</span><span>${bulk_route_link("SD YRP Lot Transfer", row.lot_transfer)}</span>
                    <span>DC</span><span>${bulk_route_link("YRP Delivery Challan", row.delivery_challan)}</span>
                </div>
            </div>
        `;
    }).join("");
...
function add_bulk_actions(frm) {
    ...
    frm.add_custom_button(__("Open Full Lay Sheet"), () => {
        frappe.set_route("Form", "SD YRP Cutting LaySheet", row.cutting_laysheet);
    }, __("Active Lot"));
    ...
    if (row.delivery_challan) {
        frm.add_custom_button(__("Open Delivery Challan"), () => {
            frappe.set_route("Form", "YRP Delivery Challan", row.delivery_challan);
        }, __("Active Lot"));
    }
...
function add_bulk_transfer_actions(frm, rows) {
    ...
    if (sharedTransfer) {
        frm.add_custom_button(
            ...
            () => frappe.set_route("Form", "SD YRP Lot Transfer", transferNames[0]),
            __("Bulk Lot Transfer")
        );
    }
}
...
async function generate_bulk_bundles(frm, row) {
    await save_bulk_editor(frm);
    const laysheet = await frappe.db.get_doc("SD YRP Cutting LaySheet", row.cutting_laysheet);
