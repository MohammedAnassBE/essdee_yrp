...
frappe.ui.form.on("SD YRP Cutting LaySheet", {
    setup(frm){
        ...
        if(frm.doc.status != "Cancelled"){
            ...
            if(x && (frm.doc.status == "Bundles Generated" || frm.doc.status == "Completed") ){
                if(!frm.doc.bundle_generated_date || frm.doc.bundle_generated_date == frappe.datetime.nowdate()){
                    frm.add_custom_button("Generate",()=> {
                        frappe.call({
                            method:"essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.get_parts",
                            args: {
                            ...
                            callback:function(r){
                                ...
                                let d =  new frappe.ui.Dialog({
                                    ...
                                    primary_action:async function (values){
                                        frappe.call({
                                            method:"essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.get_cut_sheet_data",
                                            args: {
            ...
            if(frm.doc.cutting_laysheet_bundles.length > 0 && frm.doc.status == "Bundles Generated" ){
                frm.add_custom_button("Print Labels", ()=> {
                    ...
                    frappe.call({
                        method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.get_piece_weight_tolerance",
                        args: {
                            doc_name: frm.doc.name
                        },
                        callback: function(r) {
                            ...
                            if (diff > tolerance && !frm.doc.approved_by) {
                                ...
                                frappe.call({
                                    method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.request_grammage_approval",
            ...
            if(frm.doc.status == "Approval Pending"){
                frappe.call({
                    method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.can_approve_grammage",
                    async: false,
                    callback: function(r) {
                        if (r.message) {
                            frm.add_custom_button(__("Approve"), ()=> {
                                frappe.call({
                                    method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.approve_grammage",
                                    args: { doc_name: frm.doc.name },
                                    freeze: true,
                ...
                frm.add_custom_button("Revert Labels", ()=> {
                    let d = new frappe.ui.Dialog({
                        ...
                        primary_action(){
                            d.hide()
                            frappe.call({
                                method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.revert_labels",
                                args : {
                                    doc_name: frm.doc.name
                                },
                                freeze: true,
                    ...
                    frm.add_custom_button("Update Status", ()=> {
                        let d = new frappe.ui.Dialog({
                            ...
                            primary_action(){
                                d.hide()
                                frappe.call({
                                    method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.update_label_print_status",
                                    args: {
                                        doc_name: frm.doc.name
                                    },
                                    freeze: true,
                ...
                frm.add_custom_button("Cancel", ()=> {
                    let d = new frappe.ui.Dialog({
                        ...
                        primary_action(){
                            d.hide()
                            frappe.call({
                                method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.cancel_laysheet",
                                args: {
                                    doc_name: frm.doc.name
                                }
                            })
    ...
    cutting_order(frm) {
        if (frm.doc.cutting_order) {
            ...
            if (frm.doc.cutting_order) {
                frappe.db.get_value("SD YRP Cutting Order", frm.doc.cutting_order, "item").then(r => {
    ...
    label_print(frm){
        frappe.call({
            method:"essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.update_cutting_plan",
            args: {
...
function print_labels(frm, printer, print_order){
    ...
    frappe.call({
        method:'essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.print_labels',
        args: args,
        callback: function(r){
            if(r.message){
                ...
                qz.print(config,[r.message.zpl]).then(()=> {
                    frappe.call({
                        method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet.sd_yrp_cutting_laysheet.mark_labels_printed",
                        args: {
                            doc_name: frm.doc.name,
                            grn: r.message.grn,
                        },
