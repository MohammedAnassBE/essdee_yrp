...
frappe.ui.form.on("SD YRP Cutting Laysheet Planner", {
	refresh(frm) {
		...
		if (!frm.is_new() && has_inputs && !is_active) {
			...
			frm.add_custom_button(label, async () => {
				...
				const response = await frappe.call({
					method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet_planner.sd_yrp_cutting_laysheet_planner.optimize",
...
async function persist_selected_strategy(frm, strategy) {
	await frappe.call({
		method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet_planner.sd_yrp_cutting_laysheet_planner.select_strategy",
		args: {
...
function start_optimization_polling(frm) {
	...
	const poll = async () => {
		...
		try {
			const response = await frappe.call({
				method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_laysheet_planner.sd_yrp_cutting_laysheet_planner.get_optimization_status",
				args: { doc_name },
			});
