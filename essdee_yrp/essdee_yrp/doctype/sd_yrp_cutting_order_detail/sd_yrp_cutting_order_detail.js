...
frappe.ui.form.on("SD YRP Cutting Order Detail", {
	setup(frm) {
			...
			frm.add_custom_button("Duplicate COD", () => {
				let d = new frappe.ui.Dialog({
					...
					primary_action(values) {
						frappe.call({
							method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_order_detail.sd_yrp_cutting_order_detail.duplicate_cod",
							args: {
								cod: frm.doc.name,
								item: values.item,
							},
							...
							callback: function(r) {
								d.hide();
								frappe.set_route("Form", "SD YRP Cutting Order Detail", r.message);
							}
						});
					},
				});
	...
	get_panel_combination(frm) {
		...
		frappe.call({
			method: 'essdee_yrp.essdee_yrp.doctype.sd_yrp_cutting_order_detail.sd_yrp_cutting_order_detail.get_co_new_combination',
			args: {
				attribute_mapping_value: frm.stiching_attribute_mapping,
				packing_attribute_details: frm.doc.attribute_values,
	...
	is_set_item(frm) {
		...
		if (frm.doc.is_set_item) {
			...
			if (!frm.doc.set_item_attribute) {
				frappe.db.get_single_value('SD YRP IPD Settings', 'default_set_item_attribute').then(val => {
					if (val) {
