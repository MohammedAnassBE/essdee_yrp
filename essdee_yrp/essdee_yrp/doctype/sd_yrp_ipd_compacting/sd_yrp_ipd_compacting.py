...
COMPACTING_KEY_FIELDS = ("cloth_item", "packing_attribute_value", "input_dia")
...
def _cloth_item_by_label(ipd_doc):
	...
	for row in ipd_doc.get("cloth_detail") or []:
		label, cloth_item = row.get("name1"), row.get("cloth")
...
def _append_detail(combinations, cloth_item, packing_value, input_dia):
	detail = {
...
def _append_accessory_details(ipd_doc, cloth_items, combinations):
	accessories = _as_json(ipd_doc.get("cloth_accessory_json")).get("items") or []
	if not accessories:
		return
	mappings = _as_json(ipd_doc.get("stiching_accessory_json")).get("items") or []
	if not mappings:
		frappe.throw(_("Configure and save the Cloth Accessory colour mapping first."))

	set_attribute = ipd_doc.get("set_item_attribute") if ipd_doc.get("is_set_item") else None
	default_cloth = _as_json(ipd_doc.get("accessory_clothtype_json"))
	by_accessory = {}
	for mapping in mappings:
		accessory = mapping.get("accessory") or mapping.get("Accessory")
		if accessory:
			by_accessory.setdefault(accessory, []).append(mapping)
	...
	for accessory_row in accessories:
		...
		matched = False
		for mapping in by_accessory.get(accessory, []):
			if set_attribute:
			...
			if not cloth_label and not ipd_doc.get("is_set_item"):
				cloth_label = default_cloth.get(accessory)
			packing_value = (
			...
			if not cloth_label or not packing_value or not cloth_item:
				frappe.throw(
					_("Complete the Cloth and Accessory Colour mapping for accessory {0}.").format(
						accessory
					)
				)
			_append_detail(combinations, cloth_item, packing_value, input_dia)
			matched = True
		if not matched:
			frappe.throw(
...
def get_expected_compacting_details(ipd_doc):
	"""Resolve the unique cloth/colour/input-dia routes defined by a garment IPD."""
	...
	for row in cutting_cloths:
		key = tuple(row.get(attribute) for attribute in cloth_attributes)
		cloth_label = row.get("Cloth")
		cloth_item = cloth_items.get(cloth_label)
		if not cloth_label or not cloth_item:
			frappe.throw(
				_("Cloth Mapping combination {0} has no valid Cloth Detail.").format(
					_format_key(key)
				)
			)
	...
	for row in cutting_items:
		key = tuple(row.get(attribute) for attribute in cloth_attributes)
		cloth_item = cloth_by_attributes.get(key)
		if not cloth_item:
			continue
		packing_value, input_dia = row.get(packing_attribute), row.get("Dia")
	...
	_append_accessory_details(ipd_doc, cloth_items, combinations)
...
def get_ipd_compacting(item_production_detail):
	name = frappe.db.exists('SD YRP IPD Compacting', {"item_production_detail": item_production_detail})
	return frappe.get_doc('SD YRP IPD Compacting', name) if name else None
...
def save_compacting_details(item_production_detail, rows, expected_modified=None):
	...
	if not compacting:
		compacting = frappe.new_doc('SD YRP IPD Compacting')
	...
	compacting.save(ignore_permissions=True)
	return _context(ipd_doc)
...
def stage_compacting_details_from_ipd(ipd_doc):
	...
	submitted = [
		row for row in payload.get("rows") or [] if compacting_key(row) in expected_keys
	]
	ipd_doc.flags.staged_compacting_details = validate_submitted_details(expected, submitted)
...
def persist_staged_compacting_details(ipd_doc):
	...
	if not compacting:
		...
		compacting.item_production_detail = ipd_doc.name
	compacting.packing_attribute = ipd_doc.packing_attribute
	compacting.set("compacting_details", ipd_doc.flags.staged_compacting_details)
	compacting.save(ignore_permissions=True)
	del ipd_doc.flags.staged_compacting_details


class SDYRPIPDCompacting(Document):
	def validate(self):
		ipd_doc = frappe.get_cached_doc('YRP Item Production Detail', self.item_production_detail)
		# Migrated IPDs can legitimately predate the parent packing-attribute
		# field. Preserve the explicit historical compacting value in that case;
		# a configured parent remains authoritative for current records.
		self.packing_attribute = ipd_doc.packing_attribute or self.packing_attribute
		seen = set()
		for row in self.get("compacting_details") or []:
			...
			if key in seen:
				frappe.throw(_("Duplicate Compacting Details combination: {0}.").format(_format_key(key)))
			seen.add(key)
		_validate_dia_values(self.get("compacting_details") or [])


IPDCompacting = SDYRPIPDCompacting
