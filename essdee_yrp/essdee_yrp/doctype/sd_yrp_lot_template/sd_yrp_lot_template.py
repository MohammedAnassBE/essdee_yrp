...
class SDYRPLotTemplate(Document):
	...
	def load_bom_attribute_list(self):
		...
		for bom in self.bom:
			if bom.attribute_mapping != None:
				doc = frappe.get_doc('YRP Item BOM Attribute Mapping', bom.attribute_mapping)
	...
	def validate(self):
		...
		if self.get('__islocal'):
			...
			for bom in self.get('bom'):
				if bom.based_on_attribute_mapping and bom.attribute_mapping:
					doc = frappe.get_doc('YRP Item BOM Attribute Mapping', bom.attribute_mapping)
					duplicate_doc = frappe.new_doc('YRP Item BOM Attribute Mapping')
					duplicate_doc.values = doc.values
		...
		for bom in self.get('bom'):
			if bom.based_on_attribute_mapping and not bom.attribute_mapping:
				...
				bom.attribute_mapping = None
				doc = frappe.delete_doc('YRP Item BOM Attribute Mapping', name)
	...
	def on_trash(self):
		for attribute in self.get('item_attributes'):
			if attribute.mapping:
			# frappe.delete_doc("YRP Item Item Attribute Mapping", attribute.mapping)
				attribute.mapping = None
			
		for bom in self.get('bom'):
			if bom.based_on_attribute_mapping and bom.attribute_mapping:
			# frappe.delete_doc("YRP Item BOM Attribute Mapping", bom.attribute_mapping)
				bom.attribute_mapping = None
...
def get_attribute_values(lot_template, attributes = None):
	lot = frappe.get_doc('SD YRP Lot Template', lot_template)
	attribute_values = {}
	...
	for b in lot.bom:
		try:
			...
			if b.based_on_attribute_mapping:
				attribute_mapping = frappe.get_doc('YRP Item BOM Attribute Mapping', b.attribute_mapping)
