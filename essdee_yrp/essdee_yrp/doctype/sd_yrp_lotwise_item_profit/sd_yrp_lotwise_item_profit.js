...
frappe.ui.form.on('SD YRP Lotwise Item Profit', {
	setup: function(frm) {
	...
	item: function(frm) {
		if (frm.doc.item) {
			frappe.db.get_doc("SD YRP FG Item Master", frm.doc.item).then(result => {
	...
	set_discount_template: function(frm) {
		...
		frappe.db.get_doc("SD YRP Lotwise Item Profit Additional Cost Template", frm.doc.trade_discounts_template)
			.then(result => {
				console.log(result);
	...
	set_other_cost_template: function(frm) {
		...
		frappe.db.get_doc("SD YRP Lotwise Item Profit Additional Cost Template", frm.doc.other_costs_template)
			.then(result => {
				console.log(result);
				frm.doc.other_costs = []
				$.each(result.value || [], function(i, v) {
	...
	set_production_cost_template: function(frm) {
		...
		frappe.db.get_doc("SD YRP Lotwise Item Profit Breakup Template", frm.doc.production_cost_template)
			.then(result => {
				console.log(result);
				frm.doc.cmt = []
...
function fetch_lot_qty(frm) {
	...
	frappe.call({
		method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_lotwise_item_profit.sd_yrp_lotwise_item_profit.get_lot_qty",
		args: {
			"lot": frm.doc.lot,
...
frappe.ui.form.on('SD YRP Lotwise Item Profit Qty Rate', {
...
frappe.ui.form.on('SD YRP Lotwise Item Profit Cloth Value', {
	ratio: function(frm, cdt, cdn) {
		let row = frappe.get_doc(cdt, cdn)
		row.qty = parseInt(row.qty);
		calculate_all(frm);
...
frappe.ui.form.on('SD YRP Lotwise Item Profit Breakup', {
...
frappe.ui.form.on('SD YRP Lotwise Item Profit Additional Cost', {
