...
frappe.ui.form.on('SD YRP Product', {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		frm.add_custom_button("Release Tech Pack", () => {
			let d = new frappe.ui.Dialog({
				...
				primary_action() {
					frappe.call({
						method: "essdee_yrp.essdee_yrp.doctype.sd_yrp_product.sd_yrp_product.release_tech_pack",
						args: {
		...
		frm.add_custom_button("Print Tech Pack", () => {
			let w = window.open(
				frappe.urllib.get_full_url(
					...
					encodeURIComponent('Essdee Product Tech Pack') + "&no_letterhead=1"
				)
			);
			if (!w) {
