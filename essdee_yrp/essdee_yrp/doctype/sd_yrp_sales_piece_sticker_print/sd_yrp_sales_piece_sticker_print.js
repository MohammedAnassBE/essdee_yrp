...
frappe.ui.form.on("SD YRP Sales Piece Sticker Print", {
	refresh(frm) {
            ...
            frm.add_custom_button("Print", ()=> {
                frappe.ui.form.qz_connect()
                    ...
                    .then(function (printers) {
                        frappe.call({
                            method:'essdee_yrp.essdee_yrp.doctype.sd_yrp_box_sticker_print.sd_yrp_box_sticker_print.get_printer',
                            args: {
                                printers: printers,
                            },
                            callback: function(r){
...
function get_printers_html(printers){
	...
	for (let i = 0; i < printers.length; i++) {
		const encodedPrinter = encodeURIComponent(printers[i]);
		const printerLabel = frappe.utils.escape_html(printers[i]);
		htmlContent += `
			<tr>
				<td style="padding: 10px; border: 1px solid #ddd; font-size: 12px;"><input type="checkbox" class="printers-checkbox-${0}" data-response="${encodedPrinter}"></td>
				<td style="padding: 10px; border: 1px solid #ddd; font-size: 12px;">${printerLabel}</td>
            </tr>
        `;
    }
...
function get_printer(){
	...
	checkedCheckboxes.each(function() {
		let p = decodeURIComponent($(this).attr('data-response') || "")
        if(p!=null){
            printers_list.add(p);
        }
...
function print_labels(frm, printer){
    frappe.call({
        method:'essdee_yrp.essdee_yrp.doctype.sd_yrp_sales_piece_sticker_print.sd_yrp_sales_piece_sticker_print.get_print_format',
        args: {
