frappe.ui.form.on("SD YRP Time and Action Gantt Chart", {
    setup(frm) {
...
function loadGanttChart(frm) {
    frappe.call({
        method:"essdee_yrp.essdee_yrp.doctype.sd_yrp_time_and_action_gantt_chart.sd_yrp_time_and_action_gantt_chart.get_chart_data",
        args :{
...
function renderGanttChart(data) {
    if (!data?.length) {
        $("#gantt").replaceWith(`<div class="text-muted">${__("No pending actions found.")}</div>`);
        return;
    }
    if (!document.getElementById("essdee-time-action-gantt-style")) {
        const style = document.createElement("style");
        style.id = "essdee-time-action-gantt-style";
        ...
        custom_popup_html: function (task) {
            ...
            return `
                <div class='details-container'>
                    <h5>${escape(task.name || "")}</h5>
                    <br>
                    <p>${__("Task started on")}: ${escape(String(task.start || ""))}</p>
                    <p>${__("Expected to finish by")}: ${escape(String(task.end || ""))}</p>
                </div>
          `;
        },
    });
