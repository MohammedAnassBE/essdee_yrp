...
class SDYRPWORecut(Document):
	def onload(self):
		if not self.is_new():
			self.set_onload(
				"recut_item_details",
				group_items_for_ui(
					self.get("wo_recut_details") or [], 'YRP Work Order Deliverables'
				),
			)
	...
	def before_validate(self):
		...
		rows = ungroup_items_from_ui(grouped, 'YRP Work Order Deliverables')
		self.set(
			"wo_recut_details",
			[
				{
