...
"""Compact panel-wise editor for garment-IPD Cutting combinations.

The matrix is an entry aid only.  ``cutting_items_json`` and
``cutting_attributes`` remain the canonical inputs consumed by the Lot cloth
requirement engine.
"""
...
def _panel_colour_context(doc, panel_values, source_packing_values):
	"""Return the actual fabric colours used by each stitched panel.

	``major_attribute_value`` is the finished garment/packing colour while
	``attribute_value`` is the fabric colour consumed by that panel.  Cutting
	consumption must be entered against the latter; the former remains available
	for selecting the cloth item in the production calculation.
	"""
	...
	for panel in panel_values:
		# Existing IPDs without a stitching colour mapping remain editable. Their
		# previous packing-colour columns are retained until the mapping is filled.
...
def _merge_cutting_rows(matrix, cutting_json, context, source_schema=1):
	"""Hydrate a compact matrix from ordinary Cutting rows.

	Legacy rows commonly omit the packing attribute.  In that case their weight
	is copied across every configured packing value, giving the user the useful
	starting point expected when first enabling the matrix.
	"""
...
def make_panel_wise_matrix(doc, include_saved=True):
	...
	# The compact editor is newer and therefore wins over its expanded copy.
	if include_saved:
		_merge_saved_matrix(
...
def expand_panel_wise_matrix(matrix, context):
	...
	for panel in matrix.get("panels") or []:
		...
		if panel_value in seen_panels:
			frappe.throw(_("Duplicate panel {0} in the consumption matrix.").format(panel_value))
		seen_panels.add(panel_value)

	for panel_value in context["panel_values"]:
		panel = panel_index.get(panel_value)
		if not panel:
			frappe.throw(_("Panel {0} is missing from the consumption matrix.").format(panel_value))
...
def sync_panel_wise_consumption_matrix(doc):
	"""Server-side save hook: matrix -> ordinary Cutting combination."""
...
def get_panel_wise_consumption_matrix(doc):
	"""Return a matrix seeded from unsaved form data and existing Cutting rows."""
	...
	# Seed every UI load from canonical Cutting rows.  The compact JSON is a save
	# transport, not a second long-lived source that could become stale while the
	# feature is disabled and the ordinary Cutting editor is used.
