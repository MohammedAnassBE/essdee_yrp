...
function edit_item(idx){
    ...
    default_list.value = []
    for(const dependentAction of items.value[idx]['dependent_action']){
        default_list.value.push(dependentAction)
    }
    nextTick(()=> {
        add_action(idx)
    })
