                ...
                <div v-for="sewing_plan in Object.keys(items[lot])" :key="sewing_plan" class="plan-card">
                    <div class="plan-header">
                        ...
                        <div class="action-buttons">
                            ...
                            <button v-if="inspection_type" class="update-btn" @click="openUpdateModal(lot, sewing_plan)">
...
const handleInspectionTypeChange = (e) => {
    selected_inspection_type.value = e.target.value
}
...
const initFrappeControls = () => {
    ...
    type_control.set_value(entry_form.value.input_type)
    type_control.refresh()
    ...
    grn_type_control.set_value(entry_form.value.received_type)
...
const input_type = computed(() => {
    const type_val = entry_form.value.input_type || input_types.value[0] || ''
    const replaced = type_val.toLowerCase().replace(/ /g, "_")
    const base_key = diff.value[replaced] || ''
    diff_key.value = base_key
    const remaining_key = `${replaced}_remaining`
    return {
        display: base_key
            ? base_key.split("_").map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(" ")
            : 'Not Configured',
        base_key: base_key,
...
const submitLog = () => {
    if (!entry_form.value.date || !entry_form.value.time || !entry_form.value.work_station || !entry_form.value.input_type || !entry_form.value.received_type) {
    ...
    for (const colour of Object.keys(modal_data.value.colours)) {
        ...
        for (const size of Object.keys(modal_data.value.colours[colour].values)) {
             ...
             const already_entered = modal_data.value.colours[colour].values[size]?.[inputTypeInfo.input_key] ?? 0;
             const allowance_pct = allowances.value[inputTypeInfo.input_key] || 0;
             const allowed_remaining = Math.max(
                 0,
                 base_qty * (1 + allowance_pct / 100) - already_entered
             );
    ...
    const payload = {
        ...
        received_type: grn_type_control?.get_value() || entry_form.value.received_type,
...
const fetchData = () => {
    ...
    frappe.call({
        ...
        callback: (r) => {
            ...
            allowances.value = message.allowances || {}
            input_types.value = message.input_types || []
            inspection_type.value = message.inspection_type
            selected_inspection_type.value = message.inspection_type
            if (!entry_form.value.input_type || !input_types.value.includes(entry_form.value.input_type)) {
                entry_form.value.input_type = input_types.value[0] || ''
            }
            entry_form.value.received_type = message.default_received_type || ''
        }
    })
}
