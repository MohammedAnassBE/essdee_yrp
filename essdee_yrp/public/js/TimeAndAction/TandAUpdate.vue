...
function update_same_action(lot){
    ...
    let d = {}
    for (const colour of Object.keys(same_dict)) {
        for (const action of same_dict[colour]) {
    ...
    let final_list = []
    for (const act of Object.keys(d)) {
...
function sort(){
    for (const lot_value of Object.keys(items.value)) {
...
function update_status(data, idx){
    if(data['actions'][idx]['merge_action'] == 1){
        let lot_value = lot.get_value()
        for (const master of Object.keys(items.value[lot_value]['masters'])) {
            for (const row of items.value[lot_value]['masters'][master]['datas']) {
                for (const row1 of row['actions']) {
