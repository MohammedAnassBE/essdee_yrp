...
function get_filters() {
    ...
    frappe.call({
        method: "essdee_yrp.time_and_action.tracking.get_t_and_a_report_data",
        args: {
            "lot":lot.get_value(),
            "item": item.get_value(),
...
function create_checkpoint(data){
    ...
    frappe.call({
        method: "essdee_yrp.time_and_action.tracking.update_wo_checkpoint",
        freeze: true,
...
function get_work_order_detail(item, idx){
    frappe.call({
        method: "essdee_yrp.time_and_action.tracking.get_work_order_details",
        args: {
            detail: item
        },
        callback: function(r){
...
function update_all_work_orders(row_data, idx){
    get_date_and_reason().then((values) => {
        if (values) {
            frappe.call({
                method: "essdee_yrp.time_and_action.tracking.update_all_work_orders",
                args: {
                    lot: row_data['lot'],
                    item: row_data['item'],
