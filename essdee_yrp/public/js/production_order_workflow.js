        ...
        frm.add_custom_button("Update Price", () => {
          let d = new frappe.ui.Dialog({
            ...
            primary_action: function () {
              ...
              frappe.call({
                method:
                  "essdee_yrp.production_order_workflow.update_price",
                args: {
          ...
          frappe.call({
            method:
              "essdee_yrp.production_order_workflow.get_price_update_context",
            args: {
              production_order: frm.doc.name,
            },
            callback: function (response) {
          ...
          () => {
            ...
            let d = new frappe.ui.Dialog({
              ...
              primary_action: function () {
                ...
                frappe.call({
                  method:
                    "essdee_yrp.production_order_workflow.update_production_order_date",
                  args: {
                    production_order: frm.doc.name,
          ...
          () => {
            let d = new frappe.ui.Dialog({
              ...
              primary_action: function () {
                ...
                frappe.call({
                  method:
                    "essdee_yrp.production_order_workflow.change_status",
                  args: {
                    production_order: frm.doc.name,
          ...
          () => {
            let d = new frappe.ui.Dialog({
              ...
              primary_action: function () {
                ...
                frappe.call({
                  method:
                    "essdee_yrp.production_order_workflow.create_lot",
                  args: {
                    production_order: frm.doc.name,
        ...
        window.open(
          frappe.urllib.get_full_url(
            "/printview?doctype=Production Order" +
			  ...
			  "&format=Essdee%20Production%20Order" +
              "&trigger_print=1"
          ),
          "_blank"
        );
          ...
          () => {
            let d = new frappe.ui.Dialog({
              ...
              primary_action: function () {
                ...
                frappe.call({
                  method:
                    "essdee_yrp.production_order_workflow.link_lot",
                  args: {
                    production_order: frm.doc.name,
                    lot_name: values.lot_name,
                  },
      ...
      frm.add_custom_button(__("Approve & Submit PPO"), () => {
        frappe.confirm(
          ...
          () => {
            frappe.call({
              method:
                "essdee_yrp.production_order_workflow.approve_ppo",
        ...
        () => {
          const dialog = new frappe.ui.Dialog({
            ...
            primary_action: (values) => {
              ...
              frappe.call({
                method:
                  "essdee_yrp.production_order_workflow.request_ppo_changes",
                args: {
                  production_order: frm.doc.name,
                  reason: values.reason,
                },
  ...
  frm.add_custom_button(__("Request PPO Approval"), () => {
    frappe.confirm(__("Send this Production Order for PPO approval?"), () => {
      const send_request = () => {
        frappe.call({
          method:
            "essdee_yrp.production_order_workflow.request_ppo_approval",
          args: { production_order: frm.doc.name },
          freeze: true,
...
function can_edit_ppo_ui(frm) {
  const can_manage_saved_order = Boolean(
    (frm.doc.__onload || {}).can_manage_production_order
  );
...
function show_update_qty_ratio_dialog(frm, size_details) {
  ...
  d = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      ...
      frappe.call({
        method:
          "essdee_yrp.production_order_workflow.update_quantity_and_ratio",
        args: {
          production_order: frm.doc.name,
...
function show_approve_quantity_ratio_dialog(frm, request) {
  ...
  const d = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      frappe.call({
        method:
          "essdee_yrp.production_order_workflow.approve_quantity_and_ratio",
        args: {
          production_order: frm.doc.name,
        },
        callback: function () {
...
function show_approve_status_change_dialog(frm, request) {
  ...
  const d = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      frappe.call({
        method:
          "essdee_yrp.production_order_workflow.approve_status_change",
        args: {
          production_order: frm.doc.name,
        },
        callback: function () {
...
function show_approve_quantity_transfer_dialog(frm, request) {
  ...
  const d = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      frappe.call({
        method:
          "essdee_yrp.production_order_workflow.approve_quantity_transfer",
        args: {
          production_order: frm.doc.name,
        },
        callback: function () {
...
function show_transfer_quantity_dialog(frm, alternative_items) {
  ...
  let d = new frappe.ui.Dialog({
    ...
    primary_action: function () {
      ...
      frappe.call({
        method:
          "essdee_yrp.production_order_workflow.transfer_quantity_to_ppo",
        args: {
...
function render_production_order_editor(frm) {
  const details_field = frm.fields_dict.details_html;
  if (!details_field) return;

  if (frm.productionTable && frm.productionTable.app) {
    frm.productionTable.app.unmount();
  }
  details_field.$wrapper.empty();
  frm.productionTable = null;

  if (!frm.doc.item) return;

  const view = new frappe.production.ui.EssdeeProductionOrder(
    details_field.wrapper
  );
  const requested_item = frm.doc.item;
  const requested_name = frm.is_new()
    ? frm.doc.amended_from || null
    : frm.doc.name;
  frm.productionTable = view;

  frappe.call({
    method: "essdee_yrp.production_order_workflow.get_order_editor_context",
    args: {
      item: requested_item,
      production_order: requested_name,
    },
    callback(response) {
      if (
        frm.productionTable !== view ||
        frm.doc.item !== requested_item ||
        (!frm.is_new() && frm.doc.name !== requested_name)
      ) {
        return;
      }
      view.load_data(response.message || {}, can_edit_ppo_ui(frm));
    },
  });
}
