...
from essdee_yrp.essdee_yrp.doctype.fg_stock_entry.fg_stock_entry import FGStockEntry
from essdee_yrp.essdee_yrp.doctype.item_conversion.item_conversion import ItemConversion
from essdee_yrp.cutting.movement import apply_transaction
from essdee_yrp.essdee_yrp.doctype.stock_summary import stock_summary
...
class TestStockBusinessLogic(IntegrationTestCase):
	def test_recut_valid_issue_submit_and_cancel(self):
		plan = frappe.get_doc("Cutting Plan", "CP-2606-00038")
		cloth_row = next(
			row
		...
		self.assertEqual(
			frappe.db.count(
				"Stock Ledger Entry",
				{
					"voucher_type": "Recut and Print Panel",
					"voucher_no": doc.name,
					"is_cancelled": 0,
				},
			),
	...
	def test_fg_stock_entry_valid_receipt_submit_and_cancel(self):
		...
		linked = frappe.get_doc("Stock Entry", doc.yrp_stock_entry)
		self.assertEqual(linked.docstatus, 1)
		self.assertEqual(linked.against, "FG Stock Entry")
	...
	def test_item_conversion_valid_transfer_submit_and_cancel(self):
		...
		self.assertEqual(
			frappe.db.count(
				"Stock Ledger Entry",
				{"voucher_type": "Item Conversion", "voucher_no": doc.name},
			),
			2,
		)
	...
	def test_essdee_rows_follow_configured_dimensions(self):
		...
		self.assertEqual(
			{row["fieldname"] for row in fields["FG Stock Entry Detail"]},
			{"lot", "quality_grade"},
		)
		self.assertEqual(
			{row["fieldname"] for row in fields["Item Conversion Detail"]},
			{"lot", "quality_grade"},
		)
		self.assertEqual(
			{row["fieldname"] for row in fields["Lot Transfer Item"]},
			{"quality_grade"},
		)
	...
	def test_fg_stock_entry_builds_base_stock_entry_with_dimensions(self):
		...
		with (
			patch("essdee_yrp.essdee_yrp.doctype.fg_stock_entry.fg_stock_entry.frappe.has_permission"),
			patch("essdee_yrp.essdee_yrp.doctype.fg_stock_entry.fg_stock_entry.frappe.new_doc", return_value=stock_entry),
			patch("essdee_yrp.essdee_yrp.doctype.fg_stock_entry.fg_stock_entry.frappe.get_meta", return_value=meta),
			patch("essdee_yrp.essdee_yrp.doctype.fg_stock_entry.fg_stock_entry.get_dimension_fieldnames", return_value=["lot", "quality_grade"]),
		):
		...
		self.assertEqual(stock_entry.against, "FG Stock Entry")
