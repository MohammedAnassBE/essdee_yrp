...
def _active_lot_names():
	if not frappe.get_meta('YRP Work Order').has_field("lot"):
		return []
...
def get_metrics():
	return {
		"open_lots": {
			"label": "Open Lots",
			"doctypes": ['SD YRP Lot'],
			"compute": lambda: _count('SD YRP Lot', OPEN_LOT_FILTERS),
			"goto": lambda: {"doctype": 'SD YRP Lot', "filters": deepcopy(OPEN_LOT_FILTERS)},
			"home_queue": True,
		},
		"active_lots": {
			"label": "Active Lots",
			"doctypes": ['YRP Work Order', 'SD YRP Lot'],
...
def _calc_lot_balance(params):
	...
	for doctype in ('SD YRP Lot', 'YRP Work Order'):
		frappe.has_permission(doctype, "read", throw=True)
	if not frappe.db.exists('SD YRP Lot', lot):
		frappe.throw(_("Lot {0} not found").format(frappe.bold(lot)), frappe.DoesNotExistError)
	frappe.has_permission('SD YRP Lot', "read", doc=lot, throw=True)
	if not frappe.get_meta('YRP Work Order').has_field("lot"):
	...
	if wo_names:
		produced = _received_from_rows(
			_wo_child_rows('YRP Work Order Receivables', {"parent": ["in", wo_names]})
		)
		delivered = _received_from_rows(
			_wo_child_rows('YRP Work Order Deliverables', {"parent": ["in", wo_names]})
		)
