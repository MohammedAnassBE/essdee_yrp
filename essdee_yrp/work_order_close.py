...
def close_work_order(
	...
	"""Close using the full unconsumed quantity or fail without changing stock."""
	...
	for row in doc.get("deliverables") or []:
		...
		if bucket_balance["qty"] + QTY_TOLERANCE < reduce_stock_qty:
			frappe.throw(
				_(
					"Cannot close Work Order {0}. Row {1} requires {2} {3} of excess usage "
					"from supplier stock, but only {4} is available for the same Stock Dimensions. "
					"Record the return or correct the consumption before closing."
				).format(
					doc.name,
					row.idx,
	...
	for plan in excess_plans:
		...
		weights = sum(
			flt(row["allocation_weight"]) for row in plan["output_allocations"]
		)
		if weights <= 0:
			frappe.throw(
		...
		for index, output in enumerate(plan["output_allocations"]):
			...
			if not target or flt(target.qty) <= 0:
				frappe.throw(_("Output receipt SLE {0} is not active.").format(target_sle))
...
def _get_excess_output_allocations(work_order, deliverable, dimensions):
	"""Resolve exact submitted Essdee output receipts that used this input."""
	rows = frappe.db.sql(
		"""
		SELECT d.name, d.output_receipt_sle, d.stock_qty, d.stock_dimensions
	...
	for row in rows:
		...
		if any(
			...
			continue
		grouped[row.output_receipt_sle] = flt(
			grouped.get(row.output_receipt_sle)
		) + flt(row.stock_qty)
	if not grouped:
		frappe.throw(
			_(
				"Cannot allocate excess usage for Work Order row {0}. No submitted GRN output has matching deliverable and Stock Dimensions."
			).format(deliverable.idx or deliverable.name)
		)
	return [
		{"output_receipt_sle": sle, "allocation_weight": weight}
		for sle, weight in grouped.items()
		if weight > 0
	]
